...
from ..parser import OutputTarget
from ..parser._ir import BinaryExpr, ValueExpr, DateExpr, BoolOp, Query, GroupExpr


class ScienceDirectDSLAdapter(DSLAdapter):
    _FIELD_MAP = {
        "title": "TITLE",
        "abstract": "ABSTRACT",
        "keywords": "KEYWORDS",
        "evidence_type": "CONTENT-TYPE",
        "published": "PUB-DATE",
        "author": "AUTHORS",
    }
    _VALUE_MAP = {
    ...
    def emit_date(self, node: DateExpr) -> Any:
        ...
        lo = arrow.get(node.date_lo).format("YYYYMMDD")
        hi = arrow.get(node.date_hi)
        if hi.floor(frame="year") == hi:
            # if only the year was specified, choose the last day of the year
            hi = hi.ceil(frame="year")
        hi = hi.format("YYYYMMDD")
        expr = ""
        match node.op:
            case "after":
                expr = f"{field_name} AFT '{lo}'"
            case "before":
                expr = f"{field_name} BEF '{hi}"
            case "between":
                expr = f"({field_name} AFT {lo} AND {field_name} BEF {hi})"
            case _:
