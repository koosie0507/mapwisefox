...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            'TITLE("machine" W/5 "learning")',
        ),
        (
            'near[5]("machine", "learning") in title, abstract',
            'TITLE-ABS("machine" W/5 "learning")',
        ),
        (
            'near[5]("machine", "learning") in title & "kitten" in abstract',
            'TITLE("machine" W/5 "learning") AND ABS("kitten")',
        ),
    ],
    indirect=["query_object"],
)
...
def test_ersa_query(parse, adapter, ersa_query_text):
    ...
    assert (
        out.query
        == r'((TITLE-ABS(("entity resolution" OR "entity alignment" OR "record linkage" OR "data deduplication" OR "merge/purge" OR "entity linking" OR "entity matching") AND ("system" OR "tool*" OR "framework" OR "architect*" OR "library"))) AND (AUTHKEY("entity resolution" OR "entity alignment" OR "record linkage" OR "data deduplication" OR "merge/purge" OR "entity linking" OR "entity matching")) AND (PUBYEAR AFT 2009 AND PUBYEAR BEF 2026)) AND (LANGUAGE("english") AND DOCTYPE("ar" OR "cp") AND SUBJAREA("COMP"))'
    )
